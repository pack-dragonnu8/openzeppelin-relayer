# Using AWS KMS for Secure Solana Transaction Signing in OpenZeppelin Relayer

This example demonstrates how to use AWS KMS hosted Ed25519 private key to securely sign Solana transactions in OpenZeppelin Relayer.
...
> **Important**: Solana uses Ed25519 cryptography. Make sure to select `ECC_EDWARDS_ED25519` as the key spec. Using other key types (like `ECC_SECG_P256K1`) will result in signing failures.
...
```bash
cp examples/solana-aws-kms-signer/.env.example examples/solana-aws-kms-signer/.env
```
...
Update the `examples/solana-aws-kms-signer/config/config.json` file with your webhook configuration:
...
```bash
docker compose -f examples/solana-aws-kms-signer/docker-compose.yaml up
```
...
This should return information about your relayer, including its Solana address (base58-encoded) derived from the AWS KMS Ed25519 public key.

#### 6.2 Test Solana RPC

Test a JSON-RPC call through the relayer:

```bash
curl -X POST http://localhost:8080/api/v1/relayers/solana-example/rpc \
  -H "Content-Type: application/json" \
  -H "AUTHORIZATION: Bearer YOUR_API_KEY" \
  -d '{
    "jsonrpc": "2.0",
...
1. **Key Type Error**:
   ...
   - Solana requires Ed25519 keys; secp256k1 keys will not work
